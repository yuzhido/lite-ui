...
```bash
// 显示图片文件正常
// showFileName 配置是否显示文件名-图片仅显示文件名,文件显示文件名+文件大小
// useType配置显示普通模式还是头像模式,头像模式上传数量控制为 1,且不显示相关操作按钮
// borderRadius 控制线是外边框圆角
// fileRadius 控制显示文件的圆角
// previewSize控制显示文件大大小
// showType控制显示模式是卡片模式还是文本信息列表模式
