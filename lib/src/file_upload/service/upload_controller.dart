...
class UploadController {
  ...
  final FileInfo Function(String path) _getFile;
  final void Function(String path, UploadStatus status, {UploadResult? result}) _onFileStatusChanged;
  final void Function(String path, double progress) _onFileProgress;
  ...
  Future<void> startUpload(String path) async {
    ...
    await _executeUpload(path);
  }

  /// 开始上传所有文件
  ///
  /// 按 [UploadConfig.maxConcurrent] 并发上限逐批上传。
  Future<void> startAllUpload(Iterable<String> paths) async {
    final futures = paths.map((path) => startUpload(path));
  ...
  void cancelUpload(String path) {
    ...
    _onFileStatusChanged(path, UploadStatus.pending);
  }

  /// 执行实际上传，含重试逻辑
  Future<void> _executeUpload(String path) async {
    _cancelFlags.remove(path);

    _onFileStatusChanged(path, UploadStatus.uploading);
    ...
    try {
      for (int attempt = 0; attempt <= _config.retryCount; attempt++) {
        ...
        // 执行上传
        result = _config.mode == UploadMode.custom && _config.customUpload != null ? await _customUpload(path) : await _builtinUpload(path);
      ...
      if (result != null && result.success) {
        ...
        if (isValid) {
          _onFileStatusChanged(path, UploadStatus.success, result: result);
        } else {
          debugPrint('[UploadController] 业务校验失败: path=$path, response=${result.responseBody}');
          _onFileStatusChanged(path, UploadStatus.failed);
        }
      } else {
        debugPrint('[UploadController] 上传失败: path=$path, error=${result?.error}');
        _onFileStatusChanged(path, UploadStatus.failed);
      }
    } finally {
      _activeUploadCount--;
  ...
  /// 自定义上传
  Future<UploadResult> _customUpload(String path) async {
    final file = _getFile(path);
    try {
      return await _config.customUpload!(file, (progress) {
        if (_cancelFlags[path] == true) return;
        _onFileProgress(path, progress);
      });
    } catch (e) {
  ...
  /// 内置 HTTP 上传
  Future<UploadResult> _builtinUpload(String path) async {
    if (_config.url == null) {
