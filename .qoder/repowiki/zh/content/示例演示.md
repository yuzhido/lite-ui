...
- 更新了 DropdownChoose 组件的 API 使用方式，从 selectedValues 迁移到新的 selectedItems API
...
**更新** 示例演示页面已完全重构为 DropdownChoose 综合使用展示，新增了完整的表单验证功能，包括自定义错误消息处理和多种验证场景的实现。最新的 API 变更包括使用 selectedItems 替代 selectedValues，以及优化的回调参数顺序。
...
**更新** 示例页面现在主要展示 DropdownChoose 组件的综合使用，包括远程搜索模式和同步数据模式的完整示例，以及完整的表单验证功能。所有多选示例都已更新为使用新的 selectedItems API 和 List<SelectItem> 数据结构。
...
- [example/lib/pages/component_demo.dart:1-312](file://example/lib/pages/component_demo.dart#L1-L312)
...
**更新** DropdownChoose 组件现在是示例演示的核心，展示了完整的远程搜索和同步数据使用模式，以及完整的表单验证功能。新的 API 使用 selectedItems 替代旧的 selectedValues，提供更好的数据管理能力。
...
**更新** 新的架构重点展示 DropdownChoose 组件的两种主要使用模式：远程搜索模式和同步数据模式，每种模式都包含单选和多选的完整示例，以及完整的表单验证功能。所有多选示例都使用新的 selectedItems API 和 List<SelectItem> 数据结构。
...
**更新** SelectModal 组件现在可以通过 DropdownChoose 组件的多种使用模式来展示，包括远程搜索和同步数据的完整示例，以及完整的表单验证功能。所有多选示例都使用新的 selectedItems API。
...
**新增** 这是本次重构的核心内容，ComponentDemoPage 现在专注于展示 DropdownChoose 组件的各种使用场景，包括完整的表单验证功能。所有示例都已更新为使用新的 selectedItems API 和优化的回调参数顺序。
...
**新增** component*demo_copy.dart 保留了原有的 InputText 验证功能，作为备用演示版本。这个版本展示了完整的表单验证场景，包括各种输入类型和校验规则。注意这个版本中仍然使用旧的回调参数顺序 (values, datas, *)，主要用于对比新旧 API 的差异。

**章节来源**

- [example/lib/pages/component_demo.dart:1-312](file://example/lib/pages/component_demo.dart#L1-L312)
- [example/lib/pages/component_demo_copy.dart:1-422](file://example/lib/pages/component_demo_copy.dart#L1-L422)
...
- [example/lib/pages/component_demo.dart:56-80](file://example/lib/pages/component_demo.dart#L56-L80)
- [example/lib/pages/component_demo.dart:127-132](file://example/lib/pages/component_demo.dart#L127-L132)
...
- [example/lib/pages/component_demo.dart:18-80](file://example/lib/pages/component_demo.dart#L18-L80)
- [example/lib/pages/component_demo.dart:127-132](file://example/lib/pages/component_demo.dart#L127-L132)
- [example/lib/pages/component_demo.dart:166-171](file://example/lib/pages/component_demo.dart#L166-L171)
...
- **API 优化**：使用 selectedItems 替代 selectedValues 可以减少不必要的状态转换，提高性能表现。
...
- **API 迁移问题**：如果从旧版本升级，需要将 selectedValues 替换为 selectedItems，并将 onConfirm 回调参数从 (values, datas, \_) 更新为 (values, items, datas)。

## 结论

Lite UI 示例工程提供了清晰、完整的组件使用范式与交互演示，覆盖常见业务场景。通过模块化结构与统一导出，开发者可以快速定位与复用组件。最新的 DropdownChoose 综合示例展示了完整的远程搜索和同步数据使用模式，以及完整的表单验证功能，为开发团队提供了标准化的选择器使用方案。新的 API 改进包括 selectedItems 的使用和优化的回调参数顺序，提供了更好的开发体验和性能表现。遵循本文档的最佳实践与性能建议，可在实际项目中高效落地。
...
- 状态管理：单选模式使用 value 和 selectedItems，多选模式使用 selectedItems（新的 API）。
- 用户体验：添加适当的 loading 状态和错误处理，提供清晰的提示信息。
- 表单验证：合理使用 validator 函数，提供友好的错误提示，避免过度验证影响用户体验。
- **API 使用**：始终使用 selectedItems 替代旧的 selectedValues，确保回调参数顺序为 (values, items, datas)。
...
- [example/lib/mock/brand_list.dart:52-59](file://example/lib/mock/brand_list.dart#L52-L59)
- [example/lib/mock/category_list.dart:57-65](file://example/lib/mock/category_list.dart#L57-L65)

### API 变更对照表

**新增** 为了帮助开发者从旧版本迁移到新版本，以下是主要的 API 变更对照：

| 旧 API                          | 新 API                              | 说明                                               |
| ------------------------------- | ----------------------------------- | -------------------------------------------------- |
| `selectedValues: Set<int>`      | `selectedItems: List<SelectItem>`   | 使用完整的选择项数据替代简单的值集合               |
