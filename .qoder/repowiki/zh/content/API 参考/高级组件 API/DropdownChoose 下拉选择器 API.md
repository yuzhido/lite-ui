基于我对代码的分析，我现在了解了 DropdownChoose 组件的最新变更。让我更新 API 文档以反映这些变化：
...
- **API 重构 - 移除 selectedValues 参数，统一使用 selectedItems**：组件现在只接受 selectedItems 作为已选项数据源，简化了 API 设计
- **回调参数顺序更新**：onConfirm 回调参数从 (values, datas, items) 更新为 (values, items, datas)，将最常用的 items 参数放在中间位置
- **增强状态管理**：改进了内部逻辑，包括弹窗展开状态管理和清除功能的状态同步
- **WrapperContainer 强类型化**：重构为 WrapperContainer<V, D> 泛型类，提供更好的类型安全性
- **新增 onLabelsResolved 回调支持**：远程模式下解析出已选项 label 的映射回调
...
**最新更新**：组件已完成 API 重构，移除了 selectedValues 参数，统一使用 selectedItems 作为已选项数据源，简化了 API 设计。同时更新了 onConfirm 回调的参数顺序，将最常用的 items 参数放在中间位置，提升了开发体验。新增了增强的状态管理机制，包括弹窗展开状态和清除功能的完整支持。
...
```mermaid
graph TB
A["DropdownChoose<br/>lib/src/dropdown_choose/dropdown_choose.dart"] --> B["ModalContent<br/>lib/src/dropdown_choose/ui/select_modal_content.dart"]
B --> C["ModalContentList<br/>lib/src/dropdown_choose/ui/widgets/select_modal_content_list.dart"]
B --> D["InputSearch<br/>lib/src/widgets/input_search.dart"]
B --> E["LookChosenList<br/>lib/src/dropdown_choose/ui/widgets/selected_items_dialog.dart"]
...
- [lib/src/dropdown_choose/ui/select_modal_content.dart:22-133](file://lib/src/dropdown_choose/ui/select_modal_content.dart#L22-L133)
...
- 数据源：items（本地模式必填）
- 已选项：selectedItems（统一的数据源，替代原来的 selectedValues）
...
**更新**：组件现在维护 `_isExpanded` 和 `_cleared` 状态变量，在弹窗打开时设置为 true，关闭时重置为 false。API 已重构，移除了 selectedValues 参数，统一使用 selectedItems 作为已选项数据源。onConfirm 回调的参数顺序已更新为 (values, items, datas)。
...
DC->>SM : showModalBottomSheet(传入 items/onRemoteSearch/multiple/_modalSelectedItems...)
...
U->>L : 点击列表项
L-->>SM : onItemTap(item)
SM-->>DC : onSelect(value, item, data) / onConfirm(values, items, datas)
...
**更新**：组件已完成 API 重构，移除了 selectedValues 参数，统一使用 selectedItems 作为已选项数据源。onConfirm 回调的参数顺序已更新为 (values, items, datas)。组件现在维护弹窗展开状态和清除状态，并在 WrapperContainer 中传递相关参数以实现动态后缀图标显示和清除功能。
...
**更新**：弹窗现在通过 `_modalSelectedItems()` 方法接收正确的选中项，确保清除后弹窗能正确显示空状态。onConfirm 回调的参数顺序已更新为 (values, items, datas)。

**章节来源**

- [lib/src/dropdown_choose/ui/select_modal_content.dart:136-237](file://lib/src/dropdown_choose/ui/select_modal_content.dart#L136-L237)
- [lib/src/dropdown_choose/ui/select_modal_content.dart:324-332](file://lib/src/dropdown_choose/ui/select_modal_content.dart#L324-L332)
...
**更新**：回调签名已经增强，现在提供了更完整的数据访问能力。OnSelectChange 现在可以访问完整的 SelectItem 对象，而不仅仅是 value 和 data。OnMultiSelectConfirm 的参数顺序经过优化，将最常用的 items 参数放在中间位置。
...
**更新**：新增了 onClear 回调，支持点击关闭图标时的清除操作，提升了用户的操作体验。**最重要的是，onSelect 和 onConfirm 的回调签名已经增强，现在可以访问完整的数据项信息**。
...
- [lib/src/dropdown_choose/ui/select_modal_content.dart:182-229](file://lib/src/dropdown_choose/ui/select_modal_content.dart#L182-L229)
...
- [lib/src/dropdown_choose/ui/select_modal_content.dart:288-295](file://lib/src/dropdown_choose/ui/select_modal_content.dart#L288-L295)
...
- **状态重置**：当外部 `value/selectedItems` 发生变化时，自动重置 `_cleared` 状态
...
- **单选模式验证**：validator 接收 widget.value?.toString() 作为参数
...
**新增功能**：这是本次更新的核心特性，提供了完整的表单验证支持，能够处理各种复杂的验证场景。
...
**更新**：新增了对状态管理和清除操作的优化建议，确保清除功能和验证功能的性能影响最小化。**同时提醒开发者注意新回调签名的性能影响**。
...
**更新**：新增了对清除功能、弹窗状态同步、验证功能和回调签名相关的故障排查指导。

**章节来源**

- [lib/src/dropdown_choose/dropdown_choose.dart:40-48](file://lib/src/dropdown_choose/dropdown_choose.dart#L40-L48)
- [lib/src/dropdown_choose/ui/select_modal_content.dart:120-125](file://lib/src/dropdown_choose/ui/select_modal_content.dart#L120-L125)

## 结论

DropdownChoose 提供了完善的下拉选择能力，覆盖表单集成、本地/远程搜索、多选限制、值显示定制、禁用项与占位提示、新增与查看已选等功能。**最新版本已完成 API 重构**，移除了 selectedValues 参数，统一使用 selectedItems 作为已选项数据源，简化了 API 设计。新增的清除功能和弹窗状态管理显著提升了用户体验，使组件更加直观和易用。**最重要的改进是回调签名的增强**，现在 OnSelectChange 和 OnMultiSelectConfirm 都提供了完整的数据项支持，让开发者能够更方便地访问选中项的详细信息。通过合理的缓存与搜索策略，可在大列表与远程数据场景下获得良好性能与用户体验。

**更新总结**：API 重构和回调签名增强为开发者提供了更简洁、更强大的 API 接口，解决了表单验证后回显异常和清除后验证不生效两个关键问题。**最重要的是，统一的 selectedItems 数据和优化的回调签名使组件更加易于使用和扩展**。
...
- formLabel：表单标签
- value：单选当前值
- **selectedItems**：**已更新**，已选中项的完整数据（多选模式下用于回显勾选状态和显示 label），替代原来的 selectedValues
- items：选项列表（本地模式必填）
- prefixIcon：前置图标
- hintText：占位提示文字
- required：是否必填
...
**更新**：新增了 validator、autovalidateMode 等验证相关参数，支持完整的表单验证功能。**最重要的是，selectedValues 参数已被移除，统一使用 selectedItems**，onSelect 和 onConfirm 的回调签名已经增强。
...
如果您正在从旧版本升级到新版本，请注意以下变更：

1. **API 重构 - 移除 selectedValues 参数**：
   - 旧版本：`selectedValues: Set<V>`
   - 新版本：`selectedItems: List<SelectItem<V, D>>`
   - 迁移：将原有的 selectedValues 转换为对应的 SelectItem 列表

2. **onConfirm 回调签名变更**：
   - 旧版本：`onConfirm: (values, datas, items)`
   - 新版本：`onConfirm: (values, items, datas)`
   - 迁移：调整参数顺序，将 `items` 移到中间位置

3. **新增功能**：
   - 可以使用 `onClear` 回调实现清除功能
   - 可以使用 `validator` 参数实现自定义验证
   - 可以使用 `autovalidateMode` 控制验证触发时机

**迁移说明**：这些变更都是向后兼容的升级，主要是增强了功能而不是破坏性变更。
...
**新增示例**：展示了如何使用增强的回调签名来访问完整的数据项信息。
