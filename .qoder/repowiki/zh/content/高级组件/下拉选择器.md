...
## 简介
本文件为 Lite UI 下拉选择器组件的完整技术文档，聚焦 DropdownChoose 组件及其弹窗内容 SelectModalContent。内容涵盖单选/多选模式、搜索过滤、远程数据加载、禁用状态、受控与非受控模式、状态同步机制、数据绑定方式、完整的 API 参考、业务场景示例、性能优化策略以及用户体验设计原则（键盘导航、无障碍支持、响应式布局等）。特别强调了**全新的API设计**，组件现在 exclusively 使用 `selectedItems` 参数进行状态管理，所有回调都接收完整的 `SelectItem` 对象，提供更丰富的数据访问能力；通过重构的 WrapperContainer 系统和完善的清空功能，提供了更直观的用户交互体验。读者可据此快速集成并高效使用。
...
- **SelectModalContent**：底部弹窗内容组件，负责搜索、数据加载（本地/远程）、选中态管理、确认与查看已选、新增操作。**关键改进**：增强的数据流处理和状态同步。
...
## 结论
DropdownChoose 提供了完善的下拉选择能力，覆盖单选/多选、搜索过滤、远程数据加载、禁用状态、值显示模式、新增操作、校验与保存等功能。**关键改进**包括：完全移除了 selectedValues 参数，现在 exclusively 使用 selectedItems 进行状态管理；增强的回调签名系统，所有组件现在都接收完整的 `SelectItem` 对象，提供更丰富的数据访问能力；重构的 WrapperContainer 系统，采用类型化的 `selectItems` 参数处理完整数据；完善的清空功能，通过 SuffixIconLabel 组件提供直观的三态视觉反馈和点击清空能力。其内部通过 SelectModalContent 统一管理弹窗状态与交互，结合重构的 WrapperContainer 和 SuffixIconLabel 实现一致的表单体验和智能视觉反馈。通过合理的缓存与渲染策略，组件在性能与可用性之间取得良好平衡。推荐的三态视觉反馈系统和清空功能显著提升了用户体验，让用户能够清晰地了解当前的选择状态和交互状态。在实际业务中结合远程缓存、分页与索引优化，可获得更佳的用户体验。
